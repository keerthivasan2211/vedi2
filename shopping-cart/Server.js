...
app.use(cors({
  origin: "http://vedi2-backend.onrender.com",
  credentials: true,
}));
...
// Utility to recalculate and return detailed cart
...
app.get("/products", (req, res) => {
  const grouped = {};
  products.forEach((p) => {
    const key = p.group + " - " + p.tamilgroup;
...
// GET /cart/display — current cart details
app.get("/cart/display", (req, res) => {
  const cartDetails = getCartDetails();
  res.json({ cartDetails });
});

// PUT /cart/increase/:name — increment quantity
app.put("/cart/increase/:name", (req, res) => {
  const { name } = req.params;
  const cartItem = cart.find((i) => i.name === name);
  if (cartItem) cartItem.quantity += 1;
  else cart.push({ name, quantity: 1 });
  res.json({ success: true, cartDetails: getCartDetails() });
});

// PUT /cart/decrease/:name — decrement quantity
...
// NEW: PUT /cart/update/:name — set quantity directly
app.put("/cart/update/:name", (req, res) => {
  const { name } = req.params;
